...
rm(list=ls())
getwd()
setwd("~/GitHub/QB2023_Stoltz/2.Worksheets/3.RStudio")
```
...
> Answer 1: Density, distribution function, quantile function and random generation for the normal distribution with mean equal to mean and standard deviation equal to sd.
...
> Answer 3: Looking at the zooplankton biomass, or ZP, there seems to generally be a low (near 0) biomass. Looking at the total nitrogen concentration, or TN, there there is an increase from 1000 to 3000 micrograms per liter. 
...
> Answer 4a. The bigger ellipses are showing higher correlation, compared to the thin ellipses that are lower correlations.
> Answer 4b. Yes
> Answer 4c. One should use non-parametric methods when there are not assumptions being made about the data.
> Answer 4d. Yes, doing the tests back to back can skew the data.
> Answer 4e. False discovery rate is important for properly identifying significant p-values.
...
> Answer 5: For the upper left figure, there is a mostly random distribution around zero. In the upper right figure, there is a linear relationship between the standardized residuals and theoretical quantiles. On the bottom left, there is again a random distribution occuring. Lastly, the bottom right there is leverage greater than zero, but less than one. 
...
zoop_wo_tanknuts <- zoop[ , 3:11]
ZPmeso.num <- meso.num[ ,c(6)]
ZPzoop <- cbind(zoop_wo_tanknuts, ZPmeso.num)
pairs(ZPzoop)
#Correlations
cornew <- corr.test(ZPzoop, method = "pearson", adjust = "BH")
print(cornew, digits=3)
```

> Answer: Looking at the numbers under ZP, or total biomass, there are only three positive numbers. Those correlate with total biomass itself, CHYD, and SIMO. As a reminder, CHYD is Chydorus species and SIMO is Simocephallus species. This means, that total biomass is correlating positively with these two species.  
